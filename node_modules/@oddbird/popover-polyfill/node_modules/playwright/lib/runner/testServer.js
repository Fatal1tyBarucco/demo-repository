...
class TestServer {
  ...
  async stop() {
    ...
    await ((_this$_dispatcher = this._dispatcher) === null || _this$_dispatcher === void 0 ? void 0 : _this$_dispatcher.setInterceptStdio({
      intercept: false
    }));
...
class TestServerDispatcher {
  ...
  async _innerListTests(params) {
    ...
    const {
      config,
      error
    } = await this._loadConfig(this._configFile, overrides);
    if (!config) {
      reporter.onError(error);
      return {
        report: [],
    ...
    if (this._watchTestDir) this._globalWatcher.update([...projectDirs], [...projectOutputs], false);
    return {
      report,
      status
    };
  ...
  async setInterceptStdio(params) {
  ...
  async _loadConfig(configFile, overrides) {
    ...
    try {
      const config = await (0, _configLoader.loadConfig)({
        resolvedConfigFile,
        configDir: resolvedConfigFile === configFileOrDirectory ? _path.default.dirname(resolvedConfigFile) : configFileOrDirectory
      }, overrides);
...
async function runUIMode(configFile, options) {
  return await innerRunTestServer(configFile, options, async (server, cancelPromise) => {
    await (0, _server.installRootRedirect)(server, [], {
      ...options,
      webApp: 'uiMode.html'
    });
    if (options.host !== undefined || options.port !== undefined) {
      await (0, _server.openTraceInBrowser)(server.urlPrefix());
    } else {
      const page = await (0, _server.openTraceViewerApp)(server.urlPrefix(), 'chromium', {
...
async function innerRunTestServer(configFile, options, openUI) {
