...
class SearchSource {
  ...
  _filterTestPathsWithStats(allPaths, testPathPatterns) {
    const data = {
      stats: {
        roots: 0,
        testMatch: 0,
    ...
    if (testPathPatterns.isSet()) {
      testCases.push({
        isMatch: path => testPathPatterns.isMatch(path),
  ...
  _getAllTestPaths(testPathPatterns) {
    return this._filterTestPathsWithStats(toTests(this._context, this._context.hasteFS.getAllFiles()), testPathPatterns);
  }
  isTestFilePath(path) {
  ...
  async _getTestPaths(globalConfig, changedFiles) {
    ...
    if (globalConfig.runTestsByPath && paths && paths.length > 0) {
      ...
      return this.findMatchingTests(_jestUtil().TestPathPatterns.fromGlobalConfig(globalConfig));
    }
  }
    ...
    for (const [reporter, options] of reporters) {
      switch (reporter) {
        case 'default':
          ...
          break;
        case 'github-actions':
          _ciInfo().GITHUB_ACTIONS && this.addReporter(new (_reporters().GitHubActionsReporter)(this._globalConfig, options));
          break;
        case 'summary':
          summaryOptions = options;
          break;
...
const _run10000 = async (globalConfig, configs, hasDeprecationWarnings, outputStream, onComplete) => {
  ...
  globalConfig.watch || globalConfig.watchAll ? await runWatch(contexts, configs, hasDeprecationWarnings, globalConfig, outputStream, hasteMapInstances, filter) : await runWithoutWatch(globalConfig, contexts, outputStream, onComplete, changedFilesPromise, filter);
};
...
function formatHandleErrors(errors, config) {
  ...
  for (const err of errors) {
    ...
    const ansiFree = (0, _stripAnsi().default)(formatted);
...
function getNoTestFound(testRunData, globalConfig, willExitWith0) {
  ...
  if (globalConfig.runTestsByPath) {
    ...
    const testPathPatterns = _jestUtil().TestPathPatterns.fromGlobalConfig(globalConfig);
    dataMessage = `Pattern: ${_chalk().default.yellow(testPathPatterns.toPretty())} - 0 matches`;
  }
  if (willExitWith0) {
...
function updateGlobalConfig(globalConfig, options = {}) {
  ...
  newConfig.onlyChanged = !newConfig.watchAll && !newConfig.testNamePattern && !_jestUtil().TestPathPatterns.fromGlobalConfig(newConfig).isSet();
...
function shouldRunInBand(tests, timings, {
  ...
  return (
    ...
    !workerIdleMemoryLimit && (oneWorkerOrLess || oneTestOrLess || tests.length <= 20 && timings.length > 0 && areFastTests)
  );
}
...
async function watch(initialGlobalConfig, contexts, outputStream, hasteMapInstances, stdin = process.stdin, hooks = new (_jestWatcher().JestHook)(), filter) {
  ...
  const emitFileChange = () => {
    if (hooks.isUsed('onFileChange')) {
      const testPathPatterns = new (_jestUtil().TestPathPatterns)([], globalConfig);
      const projects = searchSources.map(({
        context,
        searchSource
      }) => ({
        config: context.config,
        testPaths: searchSource.findMatchingTests(testPathPatterns).tests.map(t => t.path)
      }));
...
module.exports = JSON.parse('{"name":"@jest/core","description":"Delightful JavaScript Testing.","version":"30.0.0-alpha.2","main":"./build/index.js","types":"./build/index.d.ts","exports":{".":{"types":"./build/index.d.ts","require":"./build/index.js","import":"./build/index.mjs","default":"./build/index.js"},"./package.json":"./package.json"},"dependencies":{"@jest/console":"workspace:*","@jest/reporters":"workspace:*","@jest/test-result":"workspace:*","@jest/transform":"workspace:*","@jest/types":"workspace:*","@types/node":"*","ansi-escapes":"^4.2.1","chalk":"^4.0.0","ci-info":"^4.0.0","exit":"^0.1.2","graceful-fs":"^4.2.9","jest-changed-files":"workspace:*","jest-config":"workspace:*","jest-haste-map":"workspace:*","jest-message-util":"workspace:*","jest-regex-util":"workspace:*","jest-resolve":"workspace:*","jest-resolve-dependencies":"workspace:*","jest-runner":"workspace:*","jest-runtime":"workspace:*","jest-snapshot":"workspace:*","jest-util":"workspace:*","jest-validate":"workspace:*","jest-watcher":"workspace:*","micromatch":"^4.0.4","pretty-format":"workspace:*","slash":"^3.0.0","strip-ansi":"^6.0.0"},"devDependencies":{"@jest/test-sequencer":"workspace:*","@jest/test-utils":"workspace:*","@types/exit":"^0.1.30","@types/graceful-fs":"^4.1.3","@types/micromatch":"^4.0.1"},"peerDependencies":{"node-notifier":"^8.0.1 || ^9.0.0 || ^10.0.0"},"peerDependenciesMeta":{"node-notifier":{"optional":true}},"engines":{"node":"^16.10.0 || ^18.12.0 || >=20.0.0"},"repository":{"type":"git","url":"https://github.com/jestjs/jest.git","directory":"packages/jest-core"},"bugs":{"url":"https://github.com/jestjs/jest/issues"},"homepage":"https://jestjs.io/","license":"MIT","keywords":["ava","babel","coverage","easy","expect","facebook","immersive","instant","jasmine","jest","jsdom","mocha","mocking","painless","qunit","runner","sandboxed","snapshot","tap","tape","test","testing","typescript","watch"],"publishConfig":{"access":"public"}}');
