...
class HtmlReporter extends _empty.default {
  constructor(options) {
    ...
    if (options._mode === 'test') process.env.PW_HTML_REPORT = '1';
  }
  printsToStdio() {
    return false;
  }
...
class HtmlBuilder {
  ...
  _processSuite(suite, fileId, projectName, path, outTests) {
    const newPath = [...path, suite.title];
    suite.entries().forEach(e => {
      if (e.type === 'test') outTests.push(this._createTestEntry(fileId, e, projectName, newPath));else this._processSuite(e, fileId, projectName, newPath, outTests);
    });
  }
  _createTestEntry(fileId, test, projectName, path) {
    const duration = test.results.reduce((a, r) => a + r.duration, 0);
    const location = this._relativeLocation(test.location);
    path = path.slice(1);
    const [file, ...titles] = test.titlePath();
    const testIdExpression = `[project=${this._projectId(test.parent)}]${(0, _utils.toPosixPath)(file)}\x1e${titles.join('\x1e')} (repeat:${test.repeatEachIndex})`;
